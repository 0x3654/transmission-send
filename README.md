...
| plex-sync | `https://0x3654.github.io/transmission-send/plex-sync.js` |
...
# Plex Sync — статус просмотра с аккаунтом Plex

Синхронизация просмотра Lampa ↔ Plex в обе стороны. Настройки: **Настройки → Plex**.

- **Привязка аккаунта** — кодом с `plex.tv/link` (OAuth PIN): модалка с кодом,
  вводите на любом устройстве, плагин ловит токен. Запасной путь — вставить
  `X-Plex-Token` вручную. Сервер определяется сам из аккаунта (LAN-адрес
  первым, затем публичный), либо адрес вручную (например `http://ip:32400`
  или tsdproxy-имя).
- **Lampa → Plex** — автоматом: досмотрели (≥90%) → `/:/scrobble`, частичный
  просмотр → `/:/progress` (позиция), сняли отметку → `/:/unscrobble`.
  Отправляется пачкой (после просмотра / раз в 5 минут / кнопкой).
- **Plex → Lampa** — «Синхронизировать сейчас» (и автоматически после
  привязки; опция «импорт при запуске»): обходит библиотеки, переносит
  просмотренное и позиции в таймлайн Lampa (прогресс-бары на карточках)
  и пополняет «Историю просмотров» (из неё строится «Продолжить просмотр»).
  Конфликты решаются по времени: новейшая отметка побеждает.

Матчинг — по TMDB: Lampa вся на TMDB, айтемы Plex (агенты Plex Movie/Series)
несут guid `tmdb://<id>`; таймлайн Lampa привязан к хешу оригинального
названия (`Utils.hash(season+episode+original_name)` для серий) — плагин
воспроизводит формулу. Если у аккаунта включён Plex «Sync Watch State»,
отметка на одном сервере сама разъедется по остальным серверам аккаунта.

Технические заметки:

- К plex.tv идут с заголовками (CORS там открыт — проверено), к серверу Plex —
  токен в query и только `Accept` из safelist: запрос «простой», preflight
  не нужен вообще.
- Токен сервера берётся из `resources` аккаунта (не аккаунтовый — работает
  и для шарящихся серверов).
- При активном облачном аккаунте CUB «История» не трогается (Favorite уходит
  в облако CUB) — таймлайн синхронизируется всё равно.
- Отладка: ключ `plex_dbg` в localStorage — последний payload старта плеера
  (какой вид карточки пришёл, распознан ли фильм/эпизод).
