...
страница с подписками и историей; лента — `/rss/<passkey>`.

Passkey — постоянный ключ NNM-Club из трекер-URL любого своего торрента
(`bt02.nnm-club.cc:2710/<passkey>/announce`), никогда не истекает. Как
считается статистика: лента отдаёт **магниты с персональным announce** юзера —
его торрент-клиент аннонсит на трекер его ключом, раздачи зачисляются ему.
Info-hash сервис берёт из магнит-ссылки на странице темы гостем (видна даже
при скрытом «Скачать»). Проверено: `download.php` по passkey не отдаёт (404),
логин по паролю требует капчу — магнит-путь единственный.

- подписки: разделы (`rss.php?f=<id>&t=1` — новые раздачи) и конкретные темы
  (`rss.php?topic=<id>` — новые посты), всё вставкой URL в вебе
- фильтры-регэкспы на подписку (включение/исключение), вкл/выкл
- «Моя подписка»: ключ, RSS-ссылка, история прошедших раздач (топ-50)
- состояние — один JSON на томе `/data` (без внешней БД); «сессия» — кука
  с самим passkey на год, ничего отдельного не заводится
- резолв info-hash параллельно (8 воркеров), кэши на TTL
- env: `NNM_BASE`, `PORT` (8356), `DATA_DIR` (/data), `TTL` (600)
    ...
    GET  /rss/{passkey}        личная лента (магниты с announce юзера)
    GET  /healthz
