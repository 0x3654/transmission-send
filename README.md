...
Модель та же, что у эталона с его TrackerId: пользователь один раз вставляет
свой **passkey** (постоянный ключ NNM-Club, виден в трекер-URL любого своего
торрента: `bt02.nnm-club.cc:2710/<passkey>/announce`) — и всё. Ни логина, ни
пароля, ни cookie, ни сессий — ничего временного, ничего протухающего.

Как считается статистика: лента отдаёт **магниты с персональным announce**
юзера — его торрент-клиент аннонсит на трекер его ключом, раздачи зачисляются
ему. Info-hash раздачи сервис берёт из магнит-ссылки на странице темы (гость
её видит, даже когда «Скачать» скрыт). Проверено: `download.php` по passkey
не отдаёт (404), логин по паролю требует капчу — магнит-путь единственный.
...
- «Моя подписка»: идентификатор, RSS-ссылка, история прошедших раздач
...
- env: `NNM_BASE`, `PORT` (8356), `DATA_DIR` (/data), `TTL` (600),
  `REGISTRATION=off` — закрыть регистрацию (для публичного VPS)
    ...
    GET  /                веб-интерфейс («Моя подписка»)
    GET  /rss/{token}     личная лента (магниты с announce юзера)
    GET  /healthz
